...
* All the AI generated content must be done on the `scratch/` directory
* Long math explanations are written in `.md` files, not in `.tex` files or in the chat
* While writing `.tex` files, use one sentence per line, which is defined as a sentence ends in a period
* Don't write in `scratch/expositions/`
